...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ── Delete a sale record (admin/superadmin only) ─────────────────────────
  ...
  <div class="section-card-body p-0">
    <div class="table-responsive">
      <table class="table mb-0" id="inventoryTable">
        <thead>
          <tr>
            <th>Supplement</th><th>Brand</th><th>Category</th>
            <th style="width:90px;">Price</th><th style="width:100px;">Stock</th>
            <th style="width:160px;">Restock</th><th style="width:100px;">Status</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td>
              <span class="<?= $stockClass ?>"><?= $stock ?> unit<?= $stock != 1 ? 's' : '' ?></span>
              <?php if ($stock === 0): ?><br><span style="font-size:10px;color:#ef4444;">Out of stock</span>
              <?php elseif ($stock <= 5): ?><br><span style="font-size:10px;color:#f59e0b;">Low stock</span>
              <?php endif; ?>
            </td>
            <td>
  ...
  <div class="section-card-body p-0">
    <div class="table-responsive">
      <table class="table mb-0" id="salesTable">
        <thead>
          <tr>
            <th>Receipt No.</th><th>Customer</th><th>Supplement</th><th>Category</th>
            <th>Qty</th><th>Price</th><th>Total</th><th>Payment</th><th>Date & Time</th><th>Action</th>
          </tr>
        </thead>
        <tbody>
...
// Build sales data JSON for export
$exportRows = [];
...
$salesDataJson = json_encode($exportRows);
...
function computeTotal() {
  ...
  document.getElementById("totalDisplay").textContent = "₱" + (price * qty).toLocaleString("en-PH", {minimumFractionDigits:2, maximumFractionDigits:2});
}

function exportSalesPDF() {
  if (!window.jspdf || !window.jspdf.jsPDF) {
    alert("PDF library not loaded. Please wait a moment and try again, or check your internet connection.");
    return;
  }
