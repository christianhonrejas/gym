...
<div class="section-card mb-4">
  <div class="section-card-header">
    <span class="section-card-title">
      <i class="fas fa-user-check me-2" style="color:var(--primary)"></i>Check-in Subscriber
    </span>
    <span style="font-size:12px;color:var(--text-muted);">Walk-in members are checked in automatically upon registration</span>
  </div>
...
// ── AJAX Subscriber Check-in ──────────────────────────────────
function doCheckin(type) {
  const btn    = document.getElementById('btnCheckSub');
  const memberId = document.getElementById('subIdInput').value.trim();
