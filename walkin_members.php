        ...
        $memberId = 'W-' . date('Ymd') . '-' . $seq;
            ...
            if (!$stmt) { $errorMsg = 'DB error: ' . $conn->error; }
                ...
                if ($stmt->execute()) {
                    ...
                    $successMsg = "Walk-in member <strong>" . htmlspecialchars($name) . "</strong> added successfully. Please check them in via Attendance Monitoring.";
                } else {
                    $stmt->close();
